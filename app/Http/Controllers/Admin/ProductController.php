...
class ProductController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($product)
                    ...
                    ->editColumn('featured',function($row){
                        if ($row->featured==1) {
                            return '<a href="#" data-id="'.$row->id.'" class="deactive_featurd"><span class="badge badge-success">active</span> </a>';
                        }else{
                            return '<a href="#" data-id="'.$row->id.'" class="active_featurd"><span class="badge badge-danger">deactive</span> </a>';
                        }
                    })
                    ->editColumn('today_deal',function($row){
                        if ($row->today_deal==1) {
                            return '<a href="#" data-id="'.$row->id.'" class="deactive_deal"><span class="badge badge-success">active</span> </a>';
                        }else{
                            return '<a href="#" data-id="'.$row->id.'" class="active_deal"><span class="badge badge-danger">deactive</span> </a>';
                        }
                    })
                    ->editColumn('status',function($row){
                        if ($row->status==1) {
                            return '<a href="#" data-id="'.$row->id.'" class="deactive_status"><span class="badge badge-success">active</span> </a>';
                        }else{
                            return '<a href="#" data-id="'.$row->id.'" class="active_status"><span class="badge badge-danger">deactive</span> </a>';
                        }
                    })
    ...
    public function store(Request $request)
    {
      ...
      // $data['trendy']=$request->trendy;
       ...
       if ($request->thumbnail) {
        ...
        $path = public_path('files/product/' . $photoname);
        Image::make($thumbnail->getRealPath())->resize(600,600)->save($path);
        ...
        if($request->hasFile('images')){
            foreach ($request->file('images') as $key => $image) {
                ...
                $path = public_path('files/product/' . $imageName);
                Image::make($image->getRealPath())->resize(600,600)->save($path);
                array_push($images, $imageName);
            }
            //$data['images'] = json_encode($images);
            $data['images']='files/product/'.(json_encode($images));
        }
    ...
    //not slider product
    public function notslideproduct($id)
    {
        DB::table('products')->where('id',$id)->update(['product_slider'=>0]);
        return response()->json('Slider Product Deactive');
    }

    //slider product
    public function activesliderproduct($id)
    {
